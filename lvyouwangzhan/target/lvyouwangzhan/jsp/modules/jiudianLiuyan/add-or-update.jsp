            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑酒店留言</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6">
                                        <label>酒店</label>
                                        <div>
                                            <select id="jiudianSelect" name="jiudianSelect"
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                   <div class="form-group  col-md-6">
                                       <label>留言内容</label>
                                       <input id="jiudianLiuyanContentupload" name="file" type="file">
...
<script>
    ...
    function upload() {
        ...
        $('#jiudianLiuyanContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('jiudianLiuyanContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#jiudianId") !=null){
               ...
               if(jiudianId == null || jiudianId =='' || jiudianId == 'null'){
                   alert("酒店id不能为空");
                   return;
               }
           }
           if($("#yonghuId") !=null){
            ...
            httpJson("jiudianLiuyan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    // 查询列表
        ...
        function jiudianSelect() {
            //填充下拉框选项
            http("jiudian/page?page=1&limit=100&sort=&order=", "GET", {}, (res) => {
                if(res.code == 0){
                    jiudianOptions = res.data.list;
                }
            });
        }

        function jiudianSelectOne(id) {
            http("jiudian/info/"+id, "GET", {}, (res) => {
                if(res.code == 0){
                ruleForm = res.data;
                jiudianShowImg();
                jiudianDataBind();
    ...
    <!-- 初始化级联表的下拉框(要根据内容修改) -->
    <!-- 初始化级联表的下拉框(要根据内容修改) -->
        function initializationjiudianSelect() {
            var jiudianSelect = document.getElementById('jiudianSelect');
            if(jiudianSelect != null && jiudianOptions != null  && jiudianOptions.length > 0 ) {
                for (var i = 0; i < jiudianOptions.length; i++) {
                        jiudianSelect.add(new Option(jiudianOptions[i].jiudianName, jiudianOptions[i].id));
                }

                $("#jiudianSelect").change(function(e) {
                        jiudianSelectOne(e.target.value);
                });
            }
    ...
    function setSelectOption() {
        ...
        <!--  级联表的下拉框回显  -->
            ...
            if(jiudianSelect != null && jiudianOptions != null  && jiudianOptions.length > 0 ) {
                for (var i = 0; i < jiudianOptions.length; i++) {
                    if (jiudianOptions[i].id == ruleForm.jiudianId) {//下拉框value对比,如果一致就赋值汉字
                        jiudianSelect.options[i+1].selected = true;
                        $("#jiudianSelect" ).selectpicker('refresh');
                    }
                }
            }
            var yonghuSelect = document.getElementById("yonghuSelect");
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.jiudianLiuyanContent != null && ruleForm.jiudianLiuyanContent != 'null' && ruleForm.jiudianLiuyanContent != '' && $("#jiudianLiuyanContentupload").length>0) {
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#jiudianLiuyanContentupload").length>0) {
            var jiudianLiuyanContentEditor = UE.getEditor('jiudianLiuyanContentEditor');
            if (jiudianLiuyanContentEditor.hasContents()) {
                $('#jiudianLiuyanContent-input').attr('value', jiudianLiuyanContentEditor.getPlainTxt());
            }
        }
    ...
    // 获取当前详情
    function getDetails() {
        var addjiudianLiuyan = window.sessionStorage.getItem("addjiudianLiuyan");
        if (addjiudianLiuyan != null && addjiudianLiuyan != "" && addjiudianLiuyan != "null") {
            window.sessionStorage.removeItem('addjiudianLiuyan');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("jiudianLiuyan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#jiudianId").val(ruleForm.jiudianId);
        $("#yonghuId").val(ruleForm.yonghuId);
        $("#jiudianLiuyanContent").val(ruleForm.jiudianLiuyanContent);
        $("#replyContent").val(ruleForm.replyContent);
        $("#insertTime-input").val(ruleForm.insertTime);

    }
    <!--  级联表的数据回显  -->
    function jiudianDataBind(){

                    <!-- 把id赋值给当前表的id-->
        $("#jiudianId").val(ruleForm.id);

        $("#jiudianName").val(ruleForm.jiudianName);
        $("#jiudianValue").val(ruleForm.jiudianValue);
        $("#jiudianWeizhi").val(ruleForm.jiudianWeizhi);
        $("#jiudianNewMoney").val(ruleForm.jiudianNewMoney);
        $("#jiudianContent").val(ruleForm.jiudianContent);
    }
